...
def arrElementFloatToInt(arr):    #我新加
    new_arr = []
    for oneArr in arr:
        new_oneArr = []
        for oneoneArr in oneArr:
            new_oneArr.append(int(oneoneArr))
        new_arr.append(new_oneArr)
    ...
    def __run(self, tr_idx, te_idx):
        ...
        for epoch in range(self.epochs):
            ...
            with torch.no_grad():
                ...
                f1 = f1_score(arrElementFloatToInt(te_labels), arrElementFloatToInt(output), average="weighted")   # 我新加
                # print(ham, one)
                ham_list.append(ham)
                one_list.append(one)
                cov_list.append(cov)
...
if __name__ == '__main__':
    k = 2  # folds     我：进度条的几分之几那个分母(原来是3)，这里可能是交叉验证次数
