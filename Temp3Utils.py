...
from loadVCF import loadVCF
import os
from tqdm import tqdm
from sklearn.metrics import euclidean_distances as eucl


def get_index(num_bags, para_k=10, seed=0):#我：得到交叉验证的训练集和测试集索引。这里para_k=10就是十折交叉验证。
    if seed == 0:
...
class MyData:
    ...
    def __pad_bags(self):
        ...
        #print(len(padded_bags))
        for one in padded_bags:
            print(len(one))
            break
        return padded_bags
    ...
    def __load_bags_labels(self, path, negative_label):
        ...
        #print(type(data))
        #print(data)
        #print(type(data['bags'][0][0][0][0]))
        #print(data['bags'][0])
        #print(len(data['bags']))
        #print(data['labels'][1000])
        #print(type(data['labels']))
        #print(type(data['labels'][1000][0]))
...
if __name__ == '__main__':
    ...
    mydata = MyData(path, measure='max')          #我：原来是max